"""원본 대시보드 HTML 을 표준 도메인 모델로 정규화한다.

외부 페이지의 정확한 마크업이 사전에 보장되지 않으므로
다음 전략을 단계적으로 시도한다.
...
3) 표 헤더에 "업종" + "EScore" 또는 "수익률" 이 함께 보이면
   주도 업종 표로 인식한다.
4) 위에 해당하지 않는 표/카드는 손실 없이 sections 에 일반 형태로 보존한다.
"""
...
def _value_after_node(node: Tag) -> Optional[str]:
    """라벨 노드 다음에 위치한 첫 번째 의미있는 텍스트(<br> 건너뜀)."""
...
def _parse_card_layout(soup: BeautifulSoup) -> list[DashboardSection]:
    """Bootstrap <div class="card"> 기반 카드 레이아웃을 섹션으로 추출.

    중첩 카드는 최상위 카드만 한 번 캡처한다.
    """
...
def _extract_full_page_section(soup: BeautifulSoup) -> Optional[DashboardSection]:
    """프런트의 UI 미러링을 위해 메인 콘텐츠 영역 HTML 을 통째로 보존."""
...
def _absolutize_urls(soup: BeautifulSoup, base_url: str) -> None:
    """모든 상대 URL 을 base_url 기준 절대 URL 로 치환한다.

    iframe srcDoc 등 별도 origin 에서 렌더해도 원본 사이트의 정적 자산
    (Bootstrap, Chart.js, 폰트, 스크립트 등) 이 정상 로드되도록 한다.
    """
...
def _extract_full_document_section(
    ...
    """원본 HTML 문서 전체(head+body, 스크립트/스타일 포함)를 보존.

    프런트에서 <iframe srcDoc> 로 띄우면 차트 등 JS 기반 요소가
    원본과 동일하게 렌더된다.
    """
    if not base_url:
        return None
...
def _try_parse_inline_json(soup: BeautifulSoup) -> Optional[dict[str, Any]]:
    for script_id in ("__DASHBOARD_DATA__", "__NEXT_DATA__", "dashboard-data"):
