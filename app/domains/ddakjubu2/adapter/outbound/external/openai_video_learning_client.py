...
SYSTEM_INSTRUCTIONS = (
    "너는 한국 주식 투자 콘텐츠를 분석하는 금융 애널리스트다. "
    ...
    "영상에 언급된 종목이 없으면 stock_insights 는 빈 배열로 반환한다."
)
...
class OpenAIVideoLearningClient(VideoLearningPort):
    """OpenAI Responses API (gpt-5-mini) 로 영상 학습 결과 stock_insights 를 추출하는 2차 패스 어댑터."""
    ...
    async def learn(self, source_video: SourceVideo) -> LearningNote:
        ...
        try:
            ...
            print(
                f"[ddakjubu2_llm] OpenAI responses 호출 실패 "
        ...
        return LearningNote(
            ...
            program_category=source_video.program_category or "전체영상",
    ...
    def _build_prompt(self, source_video: SourceVideo) -> str:
        ...
        return (
            "다음은 딱주부TV 영상 데이터와 1차 패스로 생성된 핵심 요약이다.\n\n"
            f"[영상 ID] {source_video.video_id}\n"
            f"[카테고리] {source_video.program_category or '전체영상'}\n"
        ...
        try:
            ...
            if start != -1 and end != -1 and end > start:
                try:
                    ...
                    pass
        print("[ddakjubu2_llm] LLM 응답 JSON 파싱 실패, 빈 결과 반환")
