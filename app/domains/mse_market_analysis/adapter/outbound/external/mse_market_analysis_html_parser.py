"""원본 시장분석 HTML 을 표준 도메인 모델로 정규화한다.

전략은 study_room 의 파서와 동일하다.

1) <script id="__DASHBOARD_DATA__" type="application/json"> 같은
   서버사이드 인라인 JSON 이 있으면 우선 사용한다.
2) 표(table) 의 헤더에 알려진 라벨(WSMA120, Slope120, Z180, EScore 등)이
   포함되면 종목 메트릭 표로 인식한다.
3) 표 헤더에 "업종" + "EScore" 또는 "수익률" 이 함께 보이면
   주도 업종 표로 인식한다.
4) Bootstrap <div class="card"> 카드 레이아웃을 인식해 라벨/값 페어를 추출.
5) 메인 콘텐츠 영역과 풀 HTML 문서를 별도 섹션으로 보존하여
   프런트가 iframe srcDoc 으로 시각적 미러링을 할 수 있게 한다.
"""
...
from app.domains.mse_market_analysis.domain.entity.dashboard_section import DashboardSection
from app.domains.mse_market_analysis.domain.entity.leading_industry import LeadingIndustry
from app.domains.mse_market_analysis.domain.entity.stock_metric import StockMetric
...
def _try_parse_inline_json(soup: BeautifulSoup) -> Optional[dict[str, Any]]:
    for script_id in ("__DASHBOARD_DATA__", "__NEXT_DATA__", "dashboard-data", "__MARKET_DATA__"):
...
def parse_market_analysis_html(
