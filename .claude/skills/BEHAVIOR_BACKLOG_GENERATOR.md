...
Backlog Title을 입력하면 **사용자 행동 기반 Agile Backlog**를 생성한다.
...
Actor는 반드시 **권한 상태 또는 역할이 포함되어야 한다.**
...
Backlog Title은 반드시 Actor + 행동 + 대상 구조여야 한다.  
Actor에는 반드시 권한 상태 또는 역할이 포함되어야 한다.  
Success Criteria에는 입력 정보, 시스템 처리, 출력 정보가 포함되어야 한다.  
